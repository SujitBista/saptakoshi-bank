...
import {
  ...
  updateCircularFormSchema,
  type CreateCircularSchemaValues,
  type UpdateCircularSchemaValues,
} from "@/features/circulars/circular-schema";

type CircularFormValues = CreateCircularSchemaValues | UpdateCircularSchemaValues;

interface CircularFormProps {
  mode: "create" | "edit";
  defaultValues: CircularFormValues;
  currentFileName?: string;
  submitLabel: string;
  onSubmit: (values: CircularFormValues) => Promise<void>;
  onCancel: () => void;
}
...
export function CircularForm({
  ...
  const schema = mode === "create" ? createCircularSchema : updateCircularFormSchema;

  const {
    register,
    handleSubmit,
    formState: { errors, isSubmitting },
  } = useForm<CircularFormValues>({
    resolver: zodResolver(schema),
    defaultValues,
  });

  async function handleFormSubmit(values: CircularFormValues) {
    setApiError(null);
    ...
    try {
      ...
      setApiError(
        mode === "create"
          ? "Unable to upload circular. Please try again."
          : "Unable to update circular. Please try again."
      );
    }
  }
