...
import {
  fetchOperationTrainingMaterialById,
  operationTrainingMaterialToFormValues,
...
export function AdminOperationTrainingMaterialEditContent({ id }: { id: number }) {
  ...
  return (
    <AdminLayout userEmail={user.email} userRole={user.role} onLogout={handleLogout}>
      <div className="mx-auto max-w-3xl">
        <Card>
          <CardHeader
            title="Edit Operation Training Material"
          ...
          <CardContent>
            {isLoading ? (
              ...
              <OperationTrainingMaterialForm
                mode="edit"
                defaultValues={operationTrainingMaterialToFormValues(material)}
                currentFileName={material.fileName}
                submitLabel="Save Changes"
                onSubmit={async (values) => {
                  await updateOperationTrainingMaterial(id, values);
                  router.push(`/admin/training-materials/operation/${id}`);
                }}
