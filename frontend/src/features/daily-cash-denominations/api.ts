...
export async function fetchDailyCashDenominationById(
  id: number
): Promise<DailyCashDenominationDetails> {
  const response = await apiClient<DailyCashDenominationResponse>(
    `/api/daily-cash-denominations/${id}`,
    { token: getToken() }
  );
...
export async function deleteDailyCashDenomination(id: number): Promise<void> {
  await apiClient<void>(`/api/daily-cash-denominations/${id}`, {
    method: "DELETE",
    token: getToken(),
  });
}
